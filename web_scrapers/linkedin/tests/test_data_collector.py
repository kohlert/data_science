...
class TestDataCollector(TestCase):
    ...
    def test_collect_data(self):
        ...
        df = pd.read_csv('linkedin/data/ds_lead_smid_get_summary_data')
        searches = [[str(id)] for id in df.loc[:, 'uid']]
        dfs = self.collector.collect_data(methods, searches, prefix='ds_lead_smid_', start=0)
